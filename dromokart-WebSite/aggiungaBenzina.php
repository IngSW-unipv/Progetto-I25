        ...
        if(count($rows) > 0 && !empty(trim($rows[0]))) {
            ...
            foreach($rows as $row) {
                ...
                if(count($columns) >= 3) {
                    ...
                    echo '<td> <form action="logic/aggiungiBenzina.php" method="post"> <input type="hidden" id="targa" name="targa" value="' .htmlspecialchars($columns[0]) .'"> ';
                    echo '<button type="submit">Riempi Serbatoio</button> </form></td>';
                    echo '</tr>';
                }
            }
